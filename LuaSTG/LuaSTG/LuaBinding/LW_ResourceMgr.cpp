...
void luastg::binding::ResourceManager::Register(lua_State *L) noexcept {
    struct Wrapper {
        static int SetResLoadInfo(lua_State *L) noexcept {
            ResourceMgr::SetResourceLoadingLog(static_cast<bool>(lua_toboolean(L, 1)));
            return 0;
        }

        static int SetResourceStatus(lua_State *L) noexcept {
            if (const char *s = luaL_checkstring(L, 1); strcmp(s, "global") == 0)
        ...
        static int LoadTexture(lua_State *L) noexcept {
            ...
            if (!pActivedPool->LoadTexture(name, path, lua_toboolean(L, 3) != 0))
        ...
        static int LoadTTF(lua_State *L) noexcept {
            ...
            const char *name = luaL_checkstring(L, 1);
            const char *path = luaL_checkstring(L, 2);
            const bool result = pActivedPool->LoadTTFFont(name, path, static_cast<float>(luaL_checknumber(L, 3)),
        ...
        static int SetTextureSamplerState(lua_State *L) noexcept {
            ...
            if (sampler_name == "" || sampler_name == "point+wrap" || sampler_name == "point+clamp" || sampler_name == "linear+wrap" ||
                sampler_name == "linear+clamp") {
