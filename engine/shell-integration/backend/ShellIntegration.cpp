...
namespace core {
    bool ShellIntegration::openFile(std::string_view const path, bool const warning = true) {
        try {
            ...
            if (FAILED(result)) {
                Logger::error("[core::ShellIntegration] openFile failed: SHOpenWithDialog failed (HRESULT 0x{:08x})"sv,
    ...
    bool ShellIntegration::openDirectory(std::string_view const path, bool const warning = true) {
        try {
            const auto fs_path{std::filesystem::path(u8sv(path))};
            const auto fs_abs_path{std::filesystem::absolute(fs_path.lexically_normal())};
    ...
    bool ShellIntegration::openUrl(std::string_view const url, bool const warning = true) {
        try {
            if (warning) {
